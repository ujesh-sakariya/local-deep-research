...
class IntegratedReportGenerator:
    ...
    def generate_report(self, initial_findings: List[Dict], query: str) -> Dict:
        ...
        try:
            ...
            sections = self._generate_sections(initial_findings, section_research, structure, query)
            
            # Step 4: Format final report
            report_content = self._format_final_report(sections, structure, section_research)
    ...
    def _determine_report_structure(self, findings: List[Dict], query: str) -> List[Dict]:
        ...
        return structure
    def _research_sections(self, structure: List[Dict], main_query: str) -> Dict[str, List[Dict]]:
        ...
        for section in structure:
            ...
            response = self._remove_think_tags(self.model.invoke(section_query_prompt).content)
            questions = [line.replace('Q:', '').strip() 
                        for line in response.split('\n') 
    ...
    def _generate_sections(self, initial_findings: List[Dict], 
                        section_research: Dict[str, List[Dict]],
                        structure: List[Dict], query: str) -> Dict[str, str]:
        sections = {}
        accumulated_content = ""
    ...
    def _format_final_report(self, sections: Dict[str, str], 
                            structure: List[Dict],
                            section_research: Dict[str, List[Dict]]) -> str:
