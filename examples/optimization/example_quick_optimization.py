...
def simulate_optimization(
    ...
    for i in range(n_trials):
        ...
        for param_name, param_config in param_space.items():
            if param_config.get("type") == "int":
                params[param_name] = random.randint(
                    ...
                    param_config.get("high", 5)
                )
            elif param_config.get("type") == "categorical":
                params[param_name] = random.choice(param_config.get("choices", ["standard"]))
        ...
        speed_score = 1.0 - (params.get("iterations", 1) * 0.1)  # More iterations = slower
        
        # Calculate weighted score
        combined_score = (
            quality_score * metric_weights.get("quality", 0.5) +
            speed_score * metric_weights.get("speed", 0.5)
        )
        
        logger.info(f"Trial {i}: Quality: {quality_score:.2f}, Speed: {speed_score:.2f}, Score: {combined_score:.2f}")
        
        # Update best parameters if this trial is better
        if combined_score > best_score:
            best_score = combined_score
            best_params = params.copy()
            logger.info(f"New best parameters found: {best_params} with score: {best_score:.2f}")
