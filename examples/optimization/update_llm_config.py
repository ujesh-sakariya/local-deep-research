...
def update_llm_configuration(
    ...
    try:
        from local_deep_research.utilities.db_utils import update_db_setting, get_db_setting
    except ImportError:
        logger.error("Could not import database utilities. Make sure you're in the correct directory.")
        return False
    ...
    try:
        ...
        logger.info(f"Current LLM configuration: model={current_model}, provider={current_provider}")
...
def main():
    parser = argparse.ArgumentParser(description="Update LLM configuration in the database")
    
    # Configuration options
    parser.add_argument("--model", help="LLM model name")
    parser.add_argument("--provider", help="LLM provider (e.g., 'anthropic', 'openai', 'openai_endpoint')")
    parser.add_argument("--endpoint", help="Endpoint URL for OpenRouter or similar services")
    ...
    if not any([args.model, args.provider, args.endpoint, args.api_key, args.temperature, args.reset]):
        parser.print_help()
        return 1
