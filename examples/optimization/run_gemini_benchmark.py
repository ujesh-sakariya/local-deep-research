...
def run_benchmarks(
    ...
    if not output_dir:
        output_dir = os.path.join(project_root, "benchmark_results", f"gemini_eval_{timestamp}")
    ...
    for benchmark, benchmark_results in results.items():
        if "error" in benchmark_results:
            ...
            logger.info(f"{benchmark}: Accuracy = {accuracy}, Duration = {duration:.1f}s")
    ...
    try:
        ...
        with open(summary_file, "w") as f:
            json.dump({
                ...
                "results": {
                    b: {
                        "accuracy": r.get("results", {}).get("accuracy", None) 
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run benchmarks with Gemini Flash via OpenRouter")
    
    # Benchmark configuration
    parser.add_argument("--examples", type=int, default=5, help="Number of examples for each benchmark")
    parser.add_argument("--benchmarks", nargs="+", choices=["simpleqa", "browsecomp"], 
                      help="Benchmarks to run (default: both)")
    parser.add_argument("--search-iterations", type=int, default=2, 
                      help="Number of search iterations")
    parser.add_argument("--questions-per-iteration", type=int, default=3,
                      help="Questions per iteration")
    parser.add_argument("--search-tool", default="searxng", help="Search tool to use")
    
    # API key
    parser.add_argument("--api-key", help="OpenRouter API key (optional if already in database)")
