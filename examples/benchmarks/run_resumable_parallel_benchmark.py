...
def run_resumable_benchmark(
    ...
    evaluation_file = os.path.join(output_dir, f"{dataset_type}_{timestamp}_evaluation.jsonl")
    ...
    if resume_from:
        ...
        if existing_results_file:
            ...
            logger.info(f"Found {len(existing_results)} existing results for {dataset_type}")
    ...
    for i, example in enumerate(dataset):
        ...
        if dataset_type.lower() == "simpleqa":
            ...
            question = example.get("problem", "")
            correct_answer = example.get("correct_answer", "") or example.get("answer", "")
        ...
        if existing_result and existing_result.get("response"):
            ...
            logger.info(f"Reusing existing result for example {i+1}/{len(dataset)}: {example_id}")
            ...
            # Process new example
            logger.info(f"Processing new example {i+1}/{len(dataset)}: {question[:50]}...")
            ...
            try:
                ...
                search_result = quick_summary(
                    ...
                    search_strategy=search_config.get("search_strategy", "source_based"),
                )
    ...
    logger.info(f"Completed {dataset_type}: {new_results_count} new, {reused_results_count} reused, {error_count} errors")
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        ...
        simpleqa_future = executor.submit(
            ...
            (args.examples, output_dir, args.resume_from, search_config, evaluation_config)
        )
        ...
        try:
            ...
            print(f"SimpleQA benchmark completed: {simpleqa_results['new_results']} new, {simpleqa_results['reused_results']} reused")
        except Exception as e:
        ...
        try:
            ...
            print(f"BrowseComp benchmark completed: {browsecomp_results['new_results']} new, {browsecomp_results['reused_results']} reused")
        except Exception as e:
