...
def kill_flask_servers():
    ...
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
            ...
            if cmdline and 'python' in proc.info['name'].lower() and any('main.py' in arg for arg in cmdline if arg):
                pid = proc.info['pid']
    ...
    if killed_pids:
        print(f"Killed Flask server processes with PIDs: {', '.join(map(str, killed_pids))}")
    else:
        ...
        if process.poll() is None:
            ...
            try:
                ...
                if os.name == 'nt':  # Windows
                    ...
                    import msvcrt
                    if process.stdout and msvcrt.get_osfhandle(process.stdout.fileno()) != -1:
                        stdout_data = process.stdout.read(1024) or ""
                    if process.stderr and msvcrt.get_osfhandle(process.stderr.fileno()) != -1:
                        stderr_data = process.stderr.read(1024) or ""
                else:  # Unix
                    if process.stdout and select.select([process.stdout], [], [], 0.5)[0]:
                        stdout_data = process.stdout.read(1024) or ""
                    if process.stderr and select.select([process.stderr], [], [], 0.5)[0]:
                        stderr_data = process.stderr.read(1024) or ""
            ...
            try:
                ...
                print("The process is running but the server may not be responding to requests")
            
            return process.pid
        else:
            stdout, stderr = process.communicate()
            print(f"Error starting Flask server. Server process exited with code {process.returncode}")
            if stdout:
...
def start_flask_server_windows(port=5000):
    ...
    try:
        ...
        print(f"Note: The process is running in a minimized window. Close that window to stop the server.")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        if sys.argv[1] == 'restart':
            ...
            print("Unknown command. Usage: python kill_servers.py [restart|start|status]")
    else:
        kill_flask_servers() 