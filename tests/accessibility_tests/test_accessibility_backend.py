...
class TestHTMLAccessibility:
    ...
    def test_form_has_proper_labels(self, research_page_html):
        ...
        for input_element in inputs:
            ...
            if input_id:
                ...
                assert label is not None, f"Input with id '{input_id}' missing associated label"
    ...
    def test_radio_button_structure(self, research_page_html):
        ...
        assert len(radio_inputs) == 2, "Should have exactly 2 research mode radio buttons"
        ...
        for mode in expected_modes:
            ...
            assert radio.get('type') == 'radio', f"Input for {mode} mode should be type='radio'"
            assert radio.get('name') == 'research_mode', f"Radio button should have name='research_mode'"
            
            # Check associated label
            label = soup.find('label', {'for': f'mode-{mode}'})
            assert label is not None, f"Missing label for {mode} mode radio button"
            assert 'mode-option' in label.get('class', []), f"Label should have mode-option class"
    ...
    def test_fieldset_and_legend(self, research_page_html):
        ...
        assert fieldset is not None, "Radio button group should be wrapped in fieldset"
        
        legend = fieldset.find('legend')
        assert legend is not None, "Fieldset should have a legend"
        assert 'Research Mode' in legend.get_text(), "Legend should describe the radio button group"
    ...
    def test_aria_attributes(self, research_page_html):
        ...
        assert radiogroup is not None, "Should have element with role='radiogroup'"
        ...
        for label in radio_labels:
            assert label.get('role') == 'radio', "Label should have role='radio'"
            assert 'aria-checked' in label.attrs, "Label should have aria-checked attribute"
            assert 'tabindex' in label.attrs, "Label should have tabindex attribute"
        
        # Check that one radio is initially checked
        checked_labels = [l for l in radio_labels if l.get('aria-checked') == 'true']
        assert len(checked_labels) == 1, "Exactly one radio should be initially checked"
    ...
    def test_screen_reader_only_class(self, research_page_html):
        ...
        for radio in radio_inputs:
            ...
            assert 'sr-only' in classes, "Radio inputs should have sr-only class"
    ...
    def test_keyboard_hints_present(self, research_page_html):
        ...
        assert 'Shift+Enter for new line' in hint_content, "Should mention Shift+Enter for new line"
    ...
    def test_form_structure(self, research_page_html):
        ...
        assert form.get('action') is None, "Form should not have action attribute (handled by JS)"
        ...
        if headings:
            ...
            for heading in headings[1:]:
                ...
                if current_level > prev_level:
                    assert current_level <= prev_level + 1, f"Heading level jumped from h{prev_level} to h{current_level}"
    ...
    def test_semantic_markup(self, research_page_html):
        ...
        assert submit_button.get('type') == 'submit', "Submit button should have type='submit'"
    ...
    def test_required_fields_marked(self, research_page_html):
        ...
        if query_field.get('required'):
            ...
            assert query_field.get('aria-required') == 'true' or query_field.has_attr('required'), \
    ...
    def test_error_handling_structure(self, research_page_html):
        ...
        assert alert_container is not None, "Should have alert container for error messages"
...
class TestAccessibilityConfiguration:
    ...
    def test_viewport_meta_tag(self, base_url):
        ...
        assert 'initial-scale=1' in content, "Viewport should include initial-scale=1"
    ...
    def test_css_focus_styles(self, base_url):
        ...
        custom_css_loaded = any('custom_dropdown.css' in href for href in css_hrefs)
