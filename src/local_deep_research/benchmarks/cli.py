...
from local_deep_research.benchmarks.comparison import compare_configurations
from local_deep_research.benchmarks.efficiency import SpeedProfiler, ResourceMonitor
...
"""
    ...
    optimize_parser.add_argument('--output-dir', default='data/optimization_results',
    ...
    optimize_parser.add_argument('--temperature', type=float, default=0.7, 
                         help='LLM temperature')
    optimize_parser.add_argument('--n-trials', type=int, default=30, 
                         help='Number of parameter combinations to try')
    optimize_parser.add_argument('--timeout', type=int, 
                         help='Maximum seconds to run optimization')
    ...
    optimize_parser.add_argument('--speed-focus', action='store_true', 
                         help='Focus optimization on speed')
    optimize_parser.add_argument('--quality-focus', action='store_true', 
    ...
    compare_parser.add_argument('--configs', required=True,
                        help='JSON file with configurations to compare')
    compare_parser.add_argument('--output-dir', default='data/benchmark_results/comparison',
                        help='Directory to save results')
    ...
    profile_parser.add_argument('--output-dir', default='data/benchmark_results/profiling',
                        help='Directory to save results')
    ...
    profile_parser.add_argument('--iterations', type=int, default=2, 
                        help='Number of search iterations')
    profile_parser.add_argument('--questions', type=int, default=2, 
                        help='Questions per iteration')
    profile_parser.add_argument('--strategy', default='iterdrag', 
...
def run_comparison(args):
    ...
    for i, result in enumerate([r for r in results['results'] if r.get('success', False)]):
...
def run_profiling(args):
    ...
    try:
        ...
        print(f"Average memory: {resource_results.get('process_memory_avg_mb', 0):.1f} MB")
        ...
        results_file = os.path.join(args.output_dir, f"profiling_results_{timestamp}.json")
