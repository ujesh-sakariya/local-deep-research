...
class AdvancedSearchSystem:
    def __init__(self):
        ...
        logger.info(
            f"AdvancedSearchSystem using search engine: {search_tool} (Type: {search_engine_type})"
        )

        # Check if search is available, log warning if not
        if self.search is None:
            logger.info(
                "WARNING: Search system initialized with no search engine! Research will not be effective."
            )
            self._update_progress(
                "WARNING: No search engine available",
                None,
                {"error": "No search engine configured properly"},
            )
    ...
    def _get_follow_up_questions(self, current_knowledge: str, query: str) -> List[str]:
        ...
        self._update_progress(
            ...
            None,
            {"iteration": len(self.questions_by_iteration)},
        )
    ...
    def _compress_knowledge(
        self, current_knowledge: str, query: str, section_links
    ) -> List[str]:
    ...
    def analyze_topic(self, query: str) -> Dict:
        ...
        while iteration < self.max_iterations:
                ...
                try:
                    ...
                    if result is not None:
                        ...
                        if settings.general.knowledge_accumulation == str(
                            ...
                            current_knowledge = self._compress_knowledge(
            ...
            try:
                formatted_findings = self._save_findings(
                    findings, current_knowledge, query
                )
            except Exception as e:
